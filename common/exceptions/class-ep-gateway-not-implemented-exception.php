...
/**
 * Ep_Gateway_Not_Implemented_Exception class
 *
 * @class   Ep_Gateway_Not_Implemented_Exception
 * @since   2.0.0
 * @package Ep_Gateway/Exceptions
 * @abstract
 */
class Ep_Gateway_Not_Implemented_Exception extends Ep_Gateway_Exception
{
    /**
     ...
     * @param int $code [optional] Error code. Default: {@see Ep_Gateway_Error::NOT_IMPLEMENTED}.
     * @param string $message [optional] Base error message.
     *                                   Default: {@see Ep_Gateway_Not_Implemented_Exception::DEFAULT_MESSAGE}.
     * @param ?Exception $previous [optional] Previous exception. Default: none.
     */
    public function __construct(
        $object,
        $interface,
        $code = Ep_Gateway_Error::NOT_IMPLEMENTED,
    ...
    protected function prepare_message()
    {
        return [
            ...
            [
                sprintf(__('Object name: %s', 'woo-flashpay'), $this->get_object()),
                WC_Log_Levels::ERROR,
            ],
            [
                sprintf(__('Interface name: %s', 'woo-flashpay'), $this->get_interface()),
                WC_Log_Levels::ERROR,
            ],
        ];
    }
