...
/**
 * Ep_Gateway_Key_Not_Found_Exception class
 *
 * @class   Ep_Gateway_Key_Not_Found_Exception
 * @since   2.0.0
 * @package Ep_Gateway/Exceptions
 */
class Ep_Gateway_Key_Not_Found_Exception extends Ep_Gateway_Exception
{
    /**
     ...
     * @param int $code [optional] Error code. Default: {@see Ep_Gateway_Error::KEY_NOT_FOUND}.
    ...
    public function __construct(
        $key,
        $available,
        $code = Ep_Gateway_Error::KEY_NOT_FOUND,
        $message = null,
        ...
        if ($message === null) {
            $message = _x('Key not found in the current array.', 'Exception message', 'woo-flashpay');
        }
    ...
    protected function prepare_message()
    {
        return [
            ...
            [
                sprintf(_x('Searched key: %s', 'Exception message', 'woo-flashpay'), $this->getKey()),
                WC_Log_Levels::ERROR,
            ],
            [
                sprintf(
                    _x('Available keys: %s', 'Exception message', 'woo-flashpay'),
