 ...
 *
 * @class    Ep_Gateway_Log
 * @since    2.0.0
 * @package  Ep_Gateway/Log
 * @category Class
 * @internal
 */
class Ep_Gateway_Log extends Ep_Gateway_Registry
{
    /**
     * <h2>Logger domain.</h2>
     *
     ...
     * @since 2.0.0
     */
    const FLASHPAY_DOMAIN = 'ep-gateway';
     ...
     * @since 2.0.0
     */
    private $domain = self::FLASHPAY_DOMAIN;
    ...
    private $masked = [
        ...
        'phone' => '***',
        'token' => '** TOKEN **',
        Ep_Gateway_Settings_General::OPTION_SECRET_KEY => '*SECRET*',
    ];
    ...
    public function start()
    {
        $this->debug(
            sprintf(
                _x('<--------------------------------------<< Running %s', 'Log information', 'woo-flashpay'),
    ...
    public function end()
    {
        $this->debug(
            sprintf(
                _x('>>--------------------------------------> Ended %s', 'Log information', 'woo-flashpay'),
                esc_url_raw(wc_get_var($_SERVER['REQUEST_URI'], ''))
            )
        );
    }
    ...
    {
        $level = flashpay()->get_general_option(Ep_Gateway_Settings_General::OPTION_LOG_LEVEL, WC_Log_Levels::DEBUG);
