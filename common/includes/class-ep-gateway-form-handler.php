...
/**
 * Ep_Gateway_Form_Handler class
 *
 * Wrapper Handle frontend forms.
 *
 * @class    Ep_Gateway_Form_Handler
 * @version  2.0.0
 * @package  Ep_Gateway/Includes
 * @category Class
 */
class Ep_Gateway_Form_Handler extends WC_Form_Handler
{
    /**
            ...
            if ($order_id === $order->get_id() && hash_equals($order->get_order_key(), $order_key) && $order->needs_payment()) {
                ...
                if ($order->needs_payment()) {
                    try {
                        ...
                        $order->set_ep_meta('_payment_method', $payment_method_id);
                        $order->set_ep_meta('_payment_method_title', $payment_method->get_title());
                        $order->save();
