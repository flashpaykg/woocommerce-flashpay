...
/**
 * Ep_Gateway_Settings_Admin class
 *
 * @class    EP_Gateway_Settings_Page
 * @version  2.0.0
 ...
 * @internal
 */
abstract class Ep_Gateway_Settings
{
    // region Constants
    ...
    // FLASHPAY Payment Page Display modes
    ...
    {
        add_filter('ep_settings_tabs_array', [$this, 'add_settings_tab'], 20);
        add_action('ep_settings_' . $this->id, [$this, 'output']);
        add_action('ep_settings_save_' . $this->id, [$this, 'save']);
    }
    ...
    {
        return apply_filters('ep_get_settings_' . $this->id, []);
    }
    ...
    {
        flashpay()->settings()->output_fields($this);
    }
    ...
    public function save()
    {
        ...
        if ($nonce === null || !wp_verify_nonce($nonce, 'woocommerce-settings')) {
            die (__('Action failed. Please refresh the page and retry.', 'woo-flashpay'));
        }

        ep_get_log()->debug('Run saving plugin settings. Section:', $this->id);

        flashpay()->settings()->save_fields($this);
    }
}