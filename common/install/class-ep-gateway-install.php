 ...
 *
 * @class    Ep_Gateway_Install
 * @version  2.0.0
 * @package  Ep_Gateway/Install
 * @category Class
 */
class Ep_Gateway_Install
{
    const UPDATE_NONCE = 'woocommerce-ep-gateway-run-upgrade-nonce';
    const FIELD_NONCE = 'nonce';
    const SETTINGS_NAME = 'woocommerce_flashpay_settings';
    const VERSION_NAME = 'woocommerce_flashpay_version';
    const MAINTENANCE = 'woocommerce_flashpay_maintenance';
    ...
    {
        add_option(self::VERSION_NAME, Ep_Core::WC_EP_VERSION);
    }
    ...
    public function update()
    {
        ...
        $this->update_version(Ep_Core::WC_EP_VERSION);
    ...
    public function show_update_warning()
    {
        ...
        ep_get_view(!$this->is_in_maintenance_mode() ? 'html-notice-update.php' : 'html-notice-upgrading.php');
    }
    ...
    public function ajax_run_upgrade()
    {
        ...
        if (
            ...
            echo json_encode(
                [
                    'status' => 'error',
                    'message' => __('You are not authorized to perform this action', 'woo-flashpay')
                ]
            );
            exit;
        }
    ...
    private function update_version($version = null)
    {
        ...
        add_option(
            self::VERSION_NAME,
            $version === null ? Ep_Core::WC_EP_VERSION : $version
        );
    }
