...
/**
 * Ep_Gateway_Signer class.
 *
 * @class    Ep_Gateway_Signer
 * @version  2.0.0
 * @package  Ep_Gateway/Modules
 * @category Class
 * @internal
 */
final class Ep_Gateway_Signer extends Ep_Gateway_Registry
{
    // region Constants
    ...
    {
        $this->secret_key = ep_is_enabled(Ep_Gateway_Settings_General::OPTION_TEST)
            ? Ep_Core::TEST_PROJECT_KEY
            : flashpay()->get_general_option(Ep_Gateway_Settings_General::OPTION_SECRET_KEY);
    }
    ...
    private function get_params_to_sign(
        ...
        foreach ($params as $key => $value) {
            ...
            if (strpos($key, self::VALUE_SEPARATOR) !== false) {
                throw new Ep_Gateway_Signature_Exception(
                    __('Key contains an invalid character', 'woo-flashpay'),
                    $key,
                    Ep_Gateway_Error::INVALID_KEY
                );
            }

            if (is_string($value) && strpos($value, self::VALUE_SEPARATOR) !== false) {
                throw new Ep_Gateway_Signature_Exception(
                    __('Value contains an invalid character', 'woo-flashpay'),
                    $value,
                    Ep_Gateway_Error::INVALID_VALUE
                );
            }
    ...
    {
        ep_get_log()->info(__('Check signature', 'woo-flashpay'));

        if (array_key_exists(self::NAME, $data)) {
            ep_get_log()->debug(__('Find signature in body data', 'woo-flashpay'));
        ...
        if (array_key_exists(self::NAME, $data[self::GENERAL])) {
            ep_get_log()->debug(__('Find signature in general data', 'woo-flashpay'));
        ...
        ep_get_log()->error(__('Not found signature', 'woo-flashpay'));
        return false;
    }
    ...
    public function sign($data)
    {
        ...
        switch (true) {
            case array_key_exists(self::GENERAL, $data):
                ep_get_log()->debug(__('Append signature to general data', 'woo-flashpay'));
                $data[self::GENERAL][self::NAME] = $signature;
                break;
            default:
                ep_get_log()->debug(__('Append signature to body data', 'woo-flashpay'));
