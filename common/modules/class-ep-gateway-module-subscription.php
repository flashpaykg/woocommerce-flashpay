...
/**
 * WC_Gateway_Flashpay_Module_Subscription class.
 *
 * @class    WC_Gateway_Flashpay_Module_Subscription
 * @version  2.0.0
 * @package  WC_Gateway_Flashpay/Modules
 * @category Class
 */
class WC_Gateway_Flashpay_Module_Subscription extends Ep_Gateway_Registry
{
    /**
     * @inheritDoc
     */
    protected function init()
    {
        ...
        add_filter(
            ...
            [$this, 'remove_failed_flashpay_attempts_meta_query'],
            10
        );
        ...
        add_action(
            'ep_scheduled_subscription_payment_after',
    ...
    {
        ep_get_log()->info('Start a subscription renewal on a schedule.');
        ep_get_log()->debug(__('Renewal order ID:', 'woo-flashpay'), $renewal_order->get_id());

        $renewal_order = new Ep_Gateway_Order($renewal_order);

        if (!$renewal_order->is_ep()) {
            ep_get_log()->debug(__('There is no FLASHPAY payment in the order. Interrupt.', 'woo-flashpay'));
            return null;
        }

        if (!$renewal_order->needs_payment()) {
            ep_get_log()->debug(__('The order does not require payment. Interrupt.', 'woo-flashpay'));
            return null;
        }

        // Get the subscription based on the renewal order
        $subscription = ep_get_subscriptions_for_renewal_order($renewal_order, true);
        ...
        $subscription_order = new Ep_Gateway_Order($subscription->get_parent_id());
        ...
        ep_get_log()->debug(__('Recurring ID:', 'woo-flashpay'), $transaction_id);

        try {
            // Create subscription instance
            $api = new Ep_Gateway_API_Subscription();
    ...
    /**
     * @param Ep_Gateway_Order $order <p>Subscription renewal order.</p>
     * @param Ep_Gateway_Info_Response $response <p>Response information.</p>
     * @return void
     * @throws Ep_Gateway_API_Exception
     * @throws Ep_Gateway_Logic_Exception
     */
    public function after_create_recurring(
        Ep_Gateway_Order $order,
        Ep_Gateway_Info_Response $response
    ) {
        if ($response->get_status() !== 'success') {
            throw new Ep_Gateway_API_Exception($response->get_message());
        }

        if (flashpay()->get_project_id() !== $response->get_project_id()) {
            throw new Ep_Gateway_Logic_Exception(__('Wrong project id.', 'woo-flashpay'));
        }

        try {
            $order->set_transaction_id($response->get_request_id());
        } catch (Exception $e) {
            throw new Ep_Gateway_Logic_Exception(__('Internal exception.', 'woo-flashpay'), 0, $e);
        }
    }
    ...
    public function subscription_cancellation($subscription)
    {
        ...
        if (!ep_is_subscription($subscription)) {
            return;
        }

        $subscription = new Ep_Gateway_Subscription($subscription);

        if (
            !apply_filters(
                'woocommerce_flashpay_allow_subscription_transaction_cancellation',
                true,
                $subscription,
                $this
            )
        ...
        $order = new Ep_Gateway_Order($subscription);
...
//        if ($payment->is_action_allowed('cancel')) {
    ...
    /**
     * Triggered when customers are changing payment method to FLASHPAY.
     *
     * @param WC_Subscription $subscription
     */
    public function on_subscription_payment_method_updated_to_flashpay($subscription)
    {
        $order = new Ep_Gateway_Order($subscription->get_id());
     ...
     * @return string
     */
    public function remove_failed_flashpay_attempts_meta_query($order_meta_query)
    {
        $order_meta_query .= " AND `meta_key` NOT IN ('" . Ep_Gateway_Order::META_FAILED_PAYMENT_COUNT . "')";
        $order_meta_query .= " AND `meta_key` NOT IN ('" . Ep_Gateway_Order::META_TRANSACTION_ID . "')";
     ...
     * @param Ep_Gateway_Subscription $subscription
     *
     * @return array
     */
    ...
    {
        $order = new Ep_Gateway_Order($subscription->get_id());
        $payment_meta['flashpay'] = [
            'post_meta' => [
                '_ep_recurring_id' => [
                    'value' => $order->get_payment_id(),
                    'label' => __('FLASHPAY Payment ID', 'woo-flashpay'),
                ],
            ],
        ];
    ...
    {
        if (!isset ($payment_meta['post_meta'][Ep_Gateway_Order::META_TRANSACTION_ID]['value'])) {
            return;
        }

        $transaction_id = $payment_meta['post_meta'][Ep_Gateway_Order::META_TRANSACTION_ID]['value'];
        $order = ep_get_order($subscription->get_id());
        ...
        $transaction = new Ep_Gateway_API_Subscription();
        ...
        $subscription->add_order_note(
            sprintf(
                __('ECOMMERCE Payment ID updated from #%d to #%d', 'woo-flashpay'),
