 ...
 * @category Class
 */
class Ep_Gateway_More extends Ep_Gateway
{
    // region Properties
    /**
     * @inheritDoc
    ...
    {
        $this->id = Ep_Gateway_Settings_More::ID;
        $this->method_title = __('FLASHPAY More payment methods', 'woo-flashpay');
        $this->method_description = __('Open the payment page with all payment methods or select an additional alternative payment method.', 'woo-flashpay');
    ...
    {
        $force = $this->get_option(Ep_Gateway_Settings::OPTION_FORCE_CODE);

        if ($force !== null && $force !== '') {
            $values = apply_filters('ep_append_force_mode', $values, $force);
        }
