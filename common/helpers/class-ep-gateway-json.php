 ...
 */
class Ep_Gateway_Json extends Ep_Gateway_Array implements JsonSerializable
{
     ...
     * @throws Ep_Gateway_Exception <p>If Json-tree path is not available in current object.</p>
     */
     ...
     * @param string $path <p>Json-tree path.</p>
     * @since 2.0.0
     * @return Ep_Gateway_Json <p>The JSON-object value by JSON-path.</p>
     * @throws Ep_Gateway_Exception <p>If Json-tree path is not available in current object.</p>
     */
     ...
     *
     * @param ?Ep_Gateway_Json &$value <p>Container for JSON-object value - variable by reference.</p>
     * @param string $path <p>Json-tree path.</p>
     * @param ?Ep_Gateway_Json $default [optional] <p>Default value if path in json-tree is not found.</p>
     * @since 2.0.0
     * @return bool <p><b>TRUE</b> if success try get string value or <b>FALSE</b> otherwise.</p>
     */
     ...
     *
     * @param string $separator [optional] <p>Tree path separator. Default: {@see Ep_Gateway_Json::PATH_SEPARATOR}.</p>
     * @param string $prefix [optional] <p>Prefix for root. Default: {@see Ep_Gateway_Json::EMPTY_STRING}.</p>
     * @since 2.0.0
     ...
     * @since 2.0.0
     * @return static
     * @throws Ep_Gateway_Duplicate_Exception
     */
     ...
     * @throws Ep_Gateway_Duplicate_Exception <p>If tree path already exists.</p>
     */
    protected function add($path, $value, $onDuplicateReplace = false)
    {
        if ($this->has($path) && !$onDuplicateReplace) {
            throw new Ep_Gateway_Duplicate_Exception($path);
        }
    ...
    protected function set($path, $value)
    {
        ...
        if (preg_match('|^(?P<array>[^]\[]+)(?P<indexes>(?:\[\d+])+)$|', $key, $match)) {
            ...
            if (count($keys) > 0) {
                if (!$child instanceof Ep_Gateway_Json) {
                    $child = new Ep_Gateway_Json();
                }
            ...
            $result = $this->try_get_object($child, $key, new Ep_Gateway_Json());
     ...
     * @since 2.0.0
     * @return void
     * @throws Ep_Gateway_Not_Available_Exception <p>Always.</p>
     * @noinspection PhpUnusedParameterInspection
     */
    protected function get($path)
    {
        throw new Ep_Gateway_Not_Available_Exception();
    }
    ...
    private function parseValue(&$value)
    {
        if (is_array($value)) {
            foreach ($value as $k => &$v) {
                if (!is_numeric($k)) {
                    $value = new Ep_Gateway_Json($value);
                    return;
                }
     ...
     * @throws Ep_Gateway_Invalid_Argument_Exception
     * @throws Ep_Gateway_Key_Not_Found_Exception
     * @throws Ep_Gateway_Not_Implemented_Exception
     */
    ...
    private function get_current_value($type, $path)
    {
        if (!$this->offsetExists($path)) {
            throw new Ep_Gateway_Key_Not_Found_Exception($path, $this->keys());
        }
        ...
        switch ($type) {
            case self::STRING_TYPE:
                if (!is_string($result)) {
                    throw new Ep_Gateway_Invalid_Argument_Exception($path, $type, gettype($result));
                }
                break;
            case self::BOOL_TYPE:
                if (!is_bool($result)) {
            ...
            case self::ARRAY_TYPE:
                if ($result instanceof Ep_Gateway_Json) {
            ...
            case self::JSON_TYPE:
                ...
                if (!$result instanceof Ep_Gateway_Json) {
                    throw new Ep_Gateway_Not_Implemented_Exception($result, Ep_Gateway_Json::class);
                }
                break;
    ...
    private function serialize($value, $key)
    {
        switch (true) {
            case array_key_exists($key, $this->packRules()):
                ...
                $transformer = Ep_Gateway_Registry::get_by_class($className);

                if ($transformer instanceof Ep_Gateway_Serializer_Interface) {
                    return $transformer->serialize($value, ...$options);
                }
                return $value;
            case $value instanceof Ep_Gateway_Array && !$value instanceof Ep_Gateway_Json:
                return $value->to_array();
            case $value instanceof Ep_Gateway_Json:
                if ($value->count() <= 0) {
                    return null;
                }
    ...
    private function serialize_array(&$array)
    {
        foreach ($array as &$item) {
            if ($item instanceof Ep_Gateway_Json) {
